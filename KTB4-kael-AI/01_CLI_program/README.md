...
### 단일 파일 구조
기능이 `list`, `search`, `show` 세 가지로 단순해 파일을 나눌 필요가 없다고 판단했습니다.
규모가 커지면 Route-Controller-Model 패턴으로 확장할 예정입니다.

### argparse 사용
터미널에서 `python3 main.py search "키워드"` 처럼 자연스러운 명령어 형태로 쓸 수 있도록 파이썬 표준 라이브러리인 `argparse`를 선택했습니다.

### DB 대신 파일 시스템 사용
별도의 데이터베이스 없이 기존에 작성하던 마크다운 파일을 그대로 활용했습니다.
노트를 저장하는 방식을 바꾸지 않아도 되기 때문에 실제로 사용하기 편리합니다.
